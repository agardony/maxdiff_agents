...
class MaxDiffLogger:
    ...
    def log_run_results(self, session: TaskSession, results: AggregatedResults, 
        ...
        run_data = {
            ...
            'personas_count': len(session.personas) if session.personas else 1,
            'multi_persona_run': bool(session.personas and len(session.personas) > 1),
    ...
    def _log_response_details(self, session: TaskSession) -> None:
        ...
        for response in session.responses:
            ...
            response_data = {
                ...
                'persona_index': response.persona_index if response.persona_index is not None else 0,
                'persona_used': session.personas[response.persona_index][:50] + '...' if session.personas and response.persona_index is not None and len(session.personas[response.persona_index]) > 50 else (session.personas[response.persona_index] if session.personas and response.persona_index is not None else 'Default'),
